# Fixed Direct Cell [16] - Corrected version that preserves original config
...
print("=== BERT Sequence Length Fix (Direct - Corrected) ===")
...
if actual_seq_len > current_max_pos:
    ...
    # Create new config preserving ALL original attributes
    print("   Creating new BERT config with preserved attributes...")
    
    # Get the original config as a dictionary
    original_config_dict = model.config.to_dict()
    
    # Update only the max_position_embeddings
    original_config_dict['max_position_embeddings'] = actual_seq_len + 100
    
    print(f"   Original max_position_embeddings: {current_max_pos}")
    print(f"   New max_position_embeddings: {original_config_dict['max_position_embeddings']}")
    
    # Create new config from the modified dictionary
    from types import SimpleNamespace
    new_config = SimpleNamespace(**original_config_dict)
        ...
        if key in new_state:
            if 'position_embeddings' in key:
                ...
                if old_pos.shape[0] <= new_pos.shape[0]:
                    ...
                    print(f"   ⚠ Cannot expand position embeddings: {old_pos.shape} vs {new_pos.shape}")
            elif old_state[key].shape == new_state[key].shape:
                new_state[key] = old_state[key]
                transferred += 1
            else:
                print(f"   ⚠ Skipped {key}: shape mismatch {old_state[key].shape} vs {new_state[key].shape}")
    
    fixed_model.load_state_dict(new_state, strict=False)
...
try:
    ...
    print("   Trying without attention mask...")
    try:
        with torch.no_grad():
        ...
        print(f"   ✗ Test failed completely: {e2}")
        raise
...
for epoch in range(epochs):
    ...
    for batch_idx, batch in enumerate(train_pbar):
        try:
            ...
            # Try with attention mask first, fallback without if needed
            try:
